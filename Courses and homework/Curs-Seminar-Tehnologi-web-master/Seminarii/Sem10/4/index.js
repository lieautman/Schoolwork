...
Student.belongsToMany(Course, {through:"enrollments"});//numele tab de legatura
...
//get course students
application.get('/universities/:universityId/courses/:courseId/enrollments',async(request,response,next)=>{
  try{
    const university = await University.findByPk(request.params.universityId);
    if(university){
      const courses = await university.getCourses({id:request.params.courseId});
...
application.post('/universities/:universityId/courses/:courseId/enrollments/:studentId',async(request,response,next)=>{
  try{
    const university = await University.findByPk(request.params.universityId);
    if(university){
      const courses = await university.getCourses({id:request.params.courseId});
      ...
      if(course&&student){
        ...
        await course.save();
        response.sendStatus(204).json({message:'merge'});
      }
      else{
        response.sendStatus(404);
      }
...
application.delete('/universities/:universityId/courses/:courseId/enrollments/:studentId',async(request,response,next)=>{
  try{
    const university = await University.findByPk(request.params.universityId);
    if(university){
      const courses = await university.getCourses({id:request.params.courseId});
...
//lista cursuri a unui student
application.get('/universities/:universityId/students/:studentId/enrollments',async(request,response,next)=>{
  try{
    const university = await University.findByPk(request.params.universityId);
    if(university){
      const students = await university.getStudents({id:request.params.studentId});
