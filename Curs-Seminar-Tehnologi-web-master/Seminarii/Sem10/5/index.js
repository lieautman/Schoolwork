...
//5 exercitiu
application.copy('/',async(request,response,next)=>{
  try{
    let obiectCreatPtIntoarcere='[{';
    const universities = await University.findAll();
    for(let u of universities){

      obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"name":"${u.name}",`;

      const students = await u.getStudents();
      if(students){
        obiectCreatPtIntoarcere+=`"students":[{`;

        for(let s of students){
          obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"key":"${s.id}",`;
          obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"firstName":"${s.firstName}",`;
          obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"lastName":"${s.lastName}",`;


          obiectCreatPtIntoarcere=obiectCreatPtIntoarcere.substr(0,obiectCreatPtIntoarcere.length-1);
          obiectCreatPtIntoarcere+='},{';

        }


        obiectCreatPtIntoarcere=obiectCreatPtIntoarcere.substr(0,obiectCreatPtIntoarcere.length-2);
        obiectCreatPtIntoarcere+='],';
      }
      const courses = await u.getCourses();
      if(courses){
        obiectCreatPtIntoarcere+=`"courses":[{`;
        for(let c of courses){
          obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"key":"${c.id}",`;
          obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"name":"${c.name}",`;
      ...
      // const enrollments = await u.getEnrollments();
      // if(enrollments){
      //   obiectCreatPtIntoarcere+=`"enrollemnts":[{`;
      //   for(let e of enrollments){
      //     obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"studentKey":"${e.idStudent}",`;
      //     obiectCreatPtIntoarcere=obiectCreatPtIntoarcere+`"courseKey":"${e.idCourse}",`;
      ...
      //   obiectCreatPtIntoarcere+='],';
      // }
      //sa iau cursurile de la univ, studentii de la cursuri, sa fac o lista pt enrollments si sa le pun separat
    ...
    const obj1=JSON.parse(obiectCreatPtIntoarcere);
    response.status(201).json(obj1);
  }
  catch(error){
    next(error);
  }
